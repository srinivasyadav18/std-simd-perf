...
bool all_of_algo(Iter first, Iter last, F&& f)
{
    ...
    while (first < lastV)
    {
        ...
        auto msk = f(tmp);
        if(!std::experimental::all_of(msk)) return false;
...
{  
    ...
    auto t1 = std::chrono::high_resolution_clock::now();
        ...
        if constexpr(!hpx::is_vectorpack_execution_policy_v<ExPolicy>)
            hpx::all_of(policy, nums.begin(), nums.end(), test_);
        else all_of_algo(nums.begin(), nums.end(), test_);
